...
v1.0.0 延续并强化每次数据库升级前的 SQLite online backup，为备份写入包含
...
suxiaoyou-backend --data-dir "/path/to/app-data" --restore-backup "/path/to/suxiaoyou.db.pre-v1.0.0-<timestamp>.bak.json"
```
...
2. 使用 v1.0.0 后端的 `--list-backups` 找到并验证 `pre-v1.0.0` 备份。
3. 继续使用 v1.0.0 后端的 `--restore-backup` 原子恢复该升级前备份。
4. 安装并启动与该备份 schema 匹配的旧版应用。

不要在恢复后再次启动 v1.0.0 再安装旧版；v1.0.0 会把已恢复的旧 schema 重新升级到当前 revision。
