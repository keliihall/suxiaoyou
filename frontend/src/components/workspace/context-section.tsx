    ...
    <div>
      ...
      <div className="px-4 py-1">
        <span className="text-[13px] text-[var(--text-secondary)]">
          {t("skillsAvailable", { count: skills.length })}
        </span>
      </div>
    </div>
  );
...
function MemoryBlock() {
  ...
  const handleSave = useCallback(() => {
    ...
    updateMutation.mutate(
      { workspace_path: workspacePath, content: editContent },
      {
        ...
        onError: () => toast.error(t("memorySaveFailed")),
      },
    );
  ...
  const handleRefresh = useCallback(() => {
    ...
    refreshMutation.mutate(workspacePath, {
      onError: () => {
        setIsRefreshing(false);
        toast.error(t("memoryRefreshFailed"));
      },
    });
  ...
  const handleExport = useCallback(() => {
    ...
    exportMutation.mutate(workspacePath, {
      ...
      onError: () => toast.error(t("memoryExportFailed")),
    });
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          <span className="mt-1 block truncate text-[12px] text-[var(--text-tertiary)]">
            {workspacePath ? t("memoryConnectorsSkills") : t("workspaceAwareContext")}
          </span>
