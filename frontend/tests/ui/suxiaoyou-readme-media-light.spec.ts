...
const artifactRoot = path.join(repoRoot, ".codex-artifacts", "suxiaoyou-readme-media-20260511");
...
const gifs = [
  ["workflow-artifacts", "suxiaoyou-workflow-artifacts.gif"],
  ["memo-to-brief", "suxiaoyou-memo-to-brief.gif"],
  ["auto-compress", "suxiaoyou-auto-compress.gif"],
] as const;

const stills = [
  "suxiaoyou-artifact-panel.png",
  "suxiaoyou-budget-analysis.png",
  "suxiaoyou-docx-brief.png",
  "suxiaoyou-long-context.png",
] as const;

test.describe("苏小有 README media capture", () => {
  test.describe.configure({ mode: "serial", timeout: 900_000 });
  test.skip(
    process.env.SUXIAOYOU_CAPTURE_README_MEDIA !== "true",
  ...
  test("record multi-file artifact workflow", async ({ page }) => {
    ...
    await saveStill(page, "suxiaoyou-artifact-panel.png");
  });
  ...
  test("record memo-to-brief workflow", async ({ page }) => {
    ...
    await saveStill(page, "suxiaoyou-docx-brief.png");
  });
  ...
  test("record spreadsheet analysis still", async ({ page }) => {
    ...
    await saveStill(page, "suxiaoyou-budget-analysis.png");
  });
  ...
  test("record long-context still", async ({ page }) => {
    ...
    await saveStill(page, "suxiaoyou-long-context.png");
  });
...
async function setupCleanLightApp(page: Page, options?: Parameters<typeof mock苏小有Api>[1]) {
