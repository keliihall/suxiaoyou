...
async function setupMockedApp(page: Page): Promise<苏小有MockState> {
...
test.describe("苏小有 conversation scale and compaction GUI workflows", () => {
  test.describe.configure({ timeout: 75_000 });

  test("manual compression workflow: context indicator starts compaction and persists the marker", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Context compression checkpoint").first(),
    ).toBeVisible();
    await expect(
      page.getByText(
    ...
    const compactResponse = page.waitForResponse(
      (res) => res.url().includes("/api/chat/compact") && res.status() === 200,
    );
    await page.getByRole("button", { name: "Click to compact now" }).click();
    await compactResponse;

    await expect(
      page.getByText("Context compressed to save tokens"),
    ).toBeVisible({ timeout: 20_000 });
    expect(state.compactRequests).toHaveLength(1);
    await expectNoAppCrash(page);
  });

  test("auto compression workflow: a long prompt stream shows compressed context in the final conversation", async ({
    page,
  }) => {
    ...
    await sendPrompt(
      page,
      "Trigger auto compress during a long context answer",
    );

    await expect(page).toHaveURL(/\/c\/session-new$/);
    await expect(
      page.getByText("Auto compacted answer persisted after compression."),
    ).toBeVisible({ timeout: 20_000 });
    await expect(
      page.getByText("Context compressed to save tokens"),
    ).toBeVisible();
  ...
  test("long conversation workflow: latest messages load first and older pages load through reverse scroll", async ({
    page,
  }) => {
    await setupMockedApp(page);
    ...
    await expect(
      page.getByText("Long conversation load test").first(),
    ).toBeVisible();
    ...
    await expect(page.getByText("Long user turn 011")).toBeVisible({
      timeout: 20_000,
    });
    await expect(page.getByText("Long assistant turn 060")).toBeVisible();
    await expectNoAppCrash(page);
  ...
  test("multi-conversation workflow: switching histories keeps each conversation isolated", async ({
    page,
  }) => {
    await setupMockedApp(page);
    ...
    await page
      .getByRole("option", { name: /Long conversation load test/i })
      .click();
