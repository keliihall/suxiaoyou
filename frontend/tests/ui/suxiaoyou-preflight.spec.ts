...
async function openNewChat(page: Page, workspace = false) {
  const path = workspace
    ? `/c/new?directory=${encodeURIComponent("/Users/alex/suxiaoyou-demo")}`
    : "/c/new";
...
const WINDOWS_TOP_ACTION_INSET = 16;
const WINDOWS_TOP_ACTION_Y =
  WINDOWS_TITLE_BAR_HEIGHT + WINDOWS_TOP_ACTION_INSET;
const WINDOWS_SIDEBAR_HEADER_HEIGHT = 48;

async function installTauriMock(page: Page, platform: DesktopPlatform) {
  await page.addInitScript((mockedPlatform: DesktopPlatform) => {
    ...
    type TauriTestWindow = Window & {
      ...
      __SUXIAOYOU_TEST_EMIT_TAURI_EVENT__: (
        event: string,
        payload: unknown,
      ) => void;
    ...
    w.__SUXIAOYOU_TEST_EMIT_TAURI_EVENT__ = (event, payload) => {
      for (const handler of listeners.get(event) ?? []) {
        callbacks.get(handler)?.({ id: 1, event, payload });
      }
    };
    w.__SUXIAOYOU_TEST_TAURI_LISTENER_COUNT__ = (event) =>
      listeners.get(event)?.length ?? 0;
  }, platform);
}

async function expectWindowsTopActionLayout(
  action: Locator,
  sidebarHeaderBottom: number,
) {
  const box = await action.boundingBox();
  expect(box).not.toBeNull();
  expect(box!.y).toBe(WINDOWS_TOP_ACTION_Y);
  expect(box!.y + box!.height).toBeLessThanOrEqual(sidebarHeaderBottom);
  await action.click({ trial: true });

  const ownsHitTarget = await action.evaluate((element) => {
...
test.describe("苏小有 UI preflight", () => {
  ...
  test("desktop chat path: Tauri native path drop attaches files and folders by local path", async ({
    ...
    await page.evaluate(
      ({ position, paths }) => {
        ...
        w.__SUXIAOYOU_TEST_EMIT_TAURI_EVENT__("tauri://drag-enter", {
          paths,
          position,
        });
        w.__SUXIAOYOU_TEST_EMIT_TAURI_EVENT__("tauri://drag-drop", {
          paths,
          position,
        });
      },
  ...
  test("desktop non-project chat opens its default working directory from the context menu", async ({
    page,
  }) => {
    ...
    const invoiceChat = page.getByRole("option", { name: /Invoice cleanup/i });
    await expect(invoiceChat).toBeVisible();
    await invoiceChat.click({ button: "right" });

    const openInFinder = page.getByRole("menuitem", {
      name: "Open in Finder",
    });
    await expect(openInFinder).toHaveCount(1);
    await expect(
      page.getByRole("menuitem", { name: "Copy working directory" }),
    ).toHaveCount(0);
  ...
  test("desktop literal-dot chat opens its default working directory from the overflow menu", async ({
    ...
    const defaultFolderChat = page.getByRole("option", {
      name: /Default folder notes/i,
    });
    await expect(defaultFolderChat).toBeVisible();
    await defaultFolderChat.hover();
    await defaultFolderChat
  ...
  test("Tauri remote non-project chat does not expose a host folder action", async ({
    ...
    const projectChat = page.getByRole("option", {
      name: /Quarterly planning notes/i,
    });
    await projectChat.click({ button: "right" });
  ...
  test("Windows desktop shell keeps top actions below title bar and clickable", async ({
    page,
  }) => {
    await installTauriMock(page, "windows");
    await seed苏小有Storage(page, { force: true });
    ...
    const searchChats = page.getByRole("button", { name: "Search chats" });
    const newChat = page.getByRole("link", { name: "New chat" });

    const sidebar = page.locator('aside[aria-label="Chat sidebar"]');
    const sidebarHeader = sidebar.locator(
      ':scope > div[aria-hidden="true"][data-tauri-drag-region]',
    );
    ...
    await expectWindowsTopActionLayout(toggleSidebar, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(searchChats, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(newChat, sidebarHeaderBottom);

    await toggleSidebar.click();
    const openSidebar = page.getByRole("button", { name: "Open sidebar" });
    await expect(openSidebar).toBeVisible();
    await expectWindowsTopActionLayout(openSidebar, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(searchChats, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(newChat, sidebarHeaderBottom);
    ...
    await expect(page.getByPlaceholder("Search chats")).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(page.getByPlaceholder("Search chats")).toBeHidden();

    await newChat.click();
    await expect(page).toHaveURL(/\/c\/new$/);
  });

  test("Windows non-chat path keeps one actionable top icon strip after collapsing the sidebar", async ({
    page,
  }) => {
    await installTauriMock(page, "windows");
    await seed苏小有Storage(page, { force: true });
    ...
    const toggleSidebar = page.getByRole("button", { name: "Toggle sidebar" });
    await expect(toggleSidebar).toHaveCount(1);
    await toggleSidebar.click();

    const openSidebar = page.getByRole("button", { name: "Open sidebar" });
    const searchChats = page.getByRole("button", { name: "Search chats" });
    const newChat = page.getByRole("link", { name: "New chat" });
    await expect(openSidebar).toHaveCount(1);
    await expect(searchChats).toHaveCount(1);
    await expect(newChat).toHaveCount(1);

    const sidebarHeaderBottom =
      WINDOWS_TITLE_BAR_HEIGHT + WINDOWS_SIDEBAR_HEADER_HEIGHT;
    expect(sidebarHeaderBottom).toBe(80);
    await expectWindowsTopActionLayout(openSidebar, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(searchChats, sidebarHeaderBottom);
    await expectWindowsTopActionLayout(newChat, sidebarHeaderBottom);
  ...
  test("desktop artifact path: artifact cards and plan review panel open from chat", async ({
    ...
    await expect(
      page.frameLocator("iframe").getByText("苏小有 GUI Preflight"),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("frontend/tests/ui/suxiaoyou-preflight.spec.ts"),
    ).toBeVisible();
  });
  ...
  test("desktop interactive path: permission request is answered through the GUI", async ({
    ...
    await expect(
      page.getByText("/Users/alex/suxiaoyou-demo/frontend"),
    ).toBeVisible();
  ...
  test("settings path: every settings tab has its primary controls", async ({
    ...
    await page.getByRole("button", { name: /alex\/suxiaoyou-demo/i }).click();
  ...
  test("plugins path: connector, plugin, skill tabs and add custom connector", async ({
    ...
    await expect(
      page.getByText(/Online skill discovery is not included in this release/i),
    ).toBeVisible();
  });
...
test.describe("苏小有 mobile remote preflight", () => {
