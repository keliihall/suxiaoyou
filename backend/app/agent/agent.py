...
BUILTIN_AGENTS: dict[str, AgentInfo] = {
    ...
    "plan": AgentInfo(
        ...
        permissions=Ruleset(rules=[
            ...
            PermissionRule(action="deny", permission="image_generate"),
            PermissionRule(action="deny", permission="office"),
            PermissionRule(action="deny", permission="restore_file_version"),
...
class AgentRegistry:
    ...
    def load_custom_agents(
        ...
        """Load custom agents from settings YAML and .suxiaoyou/agents/*.md files.

        Sources (later overrides earlier):
        1. settings.agents dict from YAML config
        2. .suxiaoyou/agents/*.md Markdown files in the project directory
        """
        ...
        # 2. Discover .suxiaoyou/agents/*.md files
        for agents_dir in [
            Path(project_dir) / ".suxiaoyou" / "agents",
