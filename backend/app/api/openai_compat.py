...
Exposes /v1/chat/completions and /v1/models so 苏小有 can be used as a
...
# Agent name prefix used in model IDs: "suxiaoyou-build" -> agent "build"
_MODEL_PREFIX = "suxiaoyou-"

# Available agents exposed as model IDs.
_AGENT_MODELS = {
    "suxiaoyou-build": {"agent": "build", "description": "Full-featured assistant with all tools"},
    "suxiaoyou-plan": {"agent": "plan", "description": "Read-only analysis and planning"},
    "suxiaoyou-explore": {"agent": "explore", "description": "Fast search and exploration"},
    "suxiaoyou-general": {"agent": "general", "description": "General-purpose assistant"},
}
...
async def chat_completions(
    ...
    Delegates to 苏小有's full agent loop (run_generation) and translates
...
async def _stream_openai_chunks(
    ...
    """Translate 苏小有 SSE events into OpenAI streaming chunks."""
    queue = job.subscribe()
    ...
    try:
        while True:
            ...
            # tool-call, tool-result, permission-request, etc. are silent — internal to 苏小有
