"""Token generation, storage, and validation for 苏小有's local API.
...
* **Session token** — generated on every backend startup, written to
  ...
  identity of the user who launched 苏小有. On a multi-user host, user
...
_REMOTE_PREFIX = "suxiaoyou_rt_"
_SESSION_PREFIX = "suxiaoyou_st_"
...
def ensure_session_token(path: Path, token: str | None = None) -> str:
    ...
    if token is None:
        ...
        raise ValueError("Session token override must use suxiaoyou_st_ prefix")
    _write_token_file(path, token)
