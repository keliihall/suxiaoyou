"""Bearer-token authentication middleware for the 苏小有 local API.

Threat model
------------

苏小有's FastAPI backend binds to ``127.0.0.1`` on an ephemeral port. The
...
session token lives in a 0600 file that only the 苏小有-running user
...
class AuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        credential_source = self._token_source(provided, session_token, settings)
        ...
        scope.setdefault("state", {})["source"] = credential_source
        await self.app(scope, receive, send)

    @staticmethod
    def _token_source(provided: str, session_token: str, settings) -> str | None:
        """Return the privilege source established by the presented token.

        Both comparisons always run to keep the timing shape uniform
        regardless of which token the client presented. Source classification
        is credential-based, not peer-IP-based: a tunnel terminates on
        loopback, so its socket peer is local even though its bearer is remote.
        """
    ...
    async def _reject(send, status: int, detail: str) -> None:
        ...
        await send({
            ...
            "headers": [
                ...
                [b"www-authenticate", b'Bearer realm="suxiaoyou"'],
            ],
        })
        await send({
