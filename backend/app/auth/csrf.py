...
苏小有's HTTP server binds to 127.0.0.1, so it is reachable from any webpage
...
def _origin_allowed(
    ...
    """Return True if ``origin`` is an 苏小有-controlled frontend origin.
...
class CsrfProtectionMiddleware:
    ...
    requests whose Origin/Referer does not match the 苏小有 frontend
    ...
    2. ``extra_allowed_origins`` passed at construction — the
       ``SUXIAOYOU_EXTRA_ALLOWED_ORIGINS`` env var, for static overrides.
    ...
    async def __call__(self, scope, receive, send):
        ...
        if source is not None:
            ...
            # certain redirects — never a legitimate 苏小有 frontend).
            if source == "null":
