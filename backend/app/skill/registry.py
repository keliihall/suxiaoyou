...
class SkillRegistry:
    ...
    Discovery order (lowest → highest priority):
      ...
      2. Global user skills  (~/.suxiaoyou/skills/)
      3. External skills     ({project}/.claude/skills/, {project}/.agents/skills/)
      4. Project skills      ({project}/.suxiaoyou/skills/)
    ...
    def scan(self, project_dir: str | None = None) -> None:
        ...
        home = Path.home()
        global_dir = home / _SUXIAOYOU_SKILL_DIR / "skills"
        ...
        if project_dir:
            ...
            # 4. Project-level .suxiaoyou/skills (highest priority)
            proj_dir = project / _SUXIAOYOU_SKILL_DIR / "skills"
            if proj_dir.is_dir():
    ...
    def _resolve_disabled_path(self) -> Path | None:
        if self._project_dir:
            return Path(self._project_dir).resolve() / ".suxiaoyou" / "skills.disabled.json"
        return Path.home() / ".suxiaoyou" / "skills.disabled.json"
