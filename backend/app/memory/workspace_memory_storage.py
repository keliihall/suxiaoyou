...
def _normalize_path(path: str) -> str:
    ...
    Collapses '.' and '..' purely lexically, preserves POSIX, drive, and UNC
    roots, and converts separators to forward slashes. This does not access the
    filesystem or resolve symlinks.
    """
    raw = path.replace("\\", "/")

    if _DRIVE_ABSOLUTE_RE.match(raw):
        return ntpath.normpath(raw).replace("\\", "/")

    if _UNC_RE.match(raw):
        return ntpath.normpath(raw).replace("\\", "/").rstrip("/")
