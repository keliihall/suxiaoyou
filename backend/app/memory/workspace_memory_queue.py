...
class WorkspaceMemoryUpdateQueue:
    ...
    def add(
        ...
        *,
        ...
        """
        canonical_path = _normalize_path(workspace_path)
        self._pending[canonical_path] = WorkspaceConversationContext(
        ...
        self._schedule_process(self._debounce_seconds)
        logger.info(
    ...
    def _schedule_process(self, delay: float) -> None:
        """Schedule queue processing after *delay*, replacing any prior timer."""
    ...
    async def _process(
        self,
        scheduled_timer: asyncio.TimerHandle | None = None,
    ) -> None:
        """Process all pending workspace memory refreshes."""
        async with self._lock:
            if scheduled_timer is not None and self._timer is not scheduled_timer:
                logger.info("Workspace memory queue: ignoring stale timer callback")
                return

            owned_timer = scheduled_timer if scheduled_timer is not None else self._timer
            if owned_timer is not None and self._timer is owned_timer:
                owned_timer.cancel()
                self._timer = None
        ...
        try:
            ...
            async with self._lock:
                self._processing = False
                retry_pending = bool(self._pending) and self._timer is None

            if retry_pending:
                self._schedule_process(0)
    ...
    async def _refresh_workspace_memory(
        ...
        # Call LLM
        response_text, usage_data, effective_model_id = await self._call_llm(
            prompt, model_id=ctx.model_id
        )
        ...
        if usage_data and ctx.session_id:
            await self._persist_usage(
                ctx.session_id, usage_data, effective_model_id
            )
    ...
    async def _call_llm(
        ...
        Returns (response_text, usage_data, effective_model_id).
        """
        ...
        try:
            ...
            if not resolved:
                ...
                return None, {}, effective_model_id
