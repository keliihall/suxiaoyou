"""苏小有 agent adapter — bridges the message bus to 苏小有's generation pipeline.

Replaces nanobot's AgentLoop: consumes InboundMessage from the bus,
runs 苏小有's full agent (run_generation), and publishes OutboundMessage.
"""
...
class AgentAdapter:
    """Consumes messages from the bus and runs 苏小有's agent pipeline.
    ...
    async def _handle(self, msg: InboundMessage) -> None:
        """Process a single inbound message through 苏小有's agent."""
    ...
    async def _call_agent(self, msg: InboundMessage) -> str:
        """Call 苏小有's run_generation pipeline and collect the response text."""
