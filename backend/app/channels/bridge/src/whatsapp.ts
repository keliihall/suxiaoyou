...
export class WhatsAppClient {
  ...
  private async downloadMedia(msg: any, mimetype?: string, fileName?: string): Promise<string | null> {
    try {
      ...
      if (fileName) {
        // Remote document names are untrusted. Normalize Windows separators,
        // keep only the final path component, and strip control characters so
        // a crafted name cannot escape the media directory.
        const prefix = `wa_${Date.now()}_${randomBytes(4).toString('hex')}_`;
        const safeLeaf = basename(fileName.replace(/\\/g, '/'))
          .replace(/[\u0000-\u001f\u007f]/g, '_')
          .trim();
        outFilename = prefix + (safeLeaf || 'document.bin');
      } else {
        const mime = mimetype || 'application/octet-stream';
        // Derive a bounded, filename-safe extension from the untrusted MIME subtype.
        const subtype = mime.split('/').pop()?.split(';')[0] || 'bin';
        const ext = '.' + (subtype.replace(/[^a-z0-9.+-]/gi, '').slice(0, 16) || 'bin');
