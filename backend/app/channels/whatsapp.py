...
from app.channels.helpers import get_bridge_install_dir, get_runtime_subdir
...
def _bridge_token_path() -> Path:
    """Return the path to the persisted bridge authentication token."""
    return get_runtime_subdir("whatsapp-auth") / "bridge-token"


def _bundled_bridge_dir() -> Path:
    """Return the source bridge shipped next to the channel modules."""
...
class WhatsAppChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        for media_path in msg.media or []:
            try:
                ...
                logger.error("Error sending WhatsApp media %s: %s", media_path, e)
                raise
    ...
    async def _handle_bridge_message(self, raw: str) -> None:
        ...
        if msg_type == "message":
            ...
            logger.info("Sender phone=%s lid=%s → sender_id=%s", phone_id or "(empty)", lid_id or "(empty)", sender_id)
...
def _ensure_bridge_setup() -> Path:
    ...
    # PyInstaller places the data files at app/channels/bridge, matching the
    # source-tree layout, so this lookup is identical in development and in a
    # frozen bundle.
    source = _bundled_bridge_dir()
    if not (source / "package.json").is_file():
        raise RuntimeError(
            "WhatsApp bridge source not found. "
            "Reinstall the application to restore its bundled resources."
        )
