...
class McpTokenStore:
    ...
    Storage path: {project_dir}/.suxiaoyou/mcp-tokens.json
    Fallback:     ~/.suxiaoyou/mcp-tokens.json
    """

    def __init__(self, project_dir: str | None = None) -> None:
        if project_dir:
            self._path = Path(project_dir).resolve() / ".suxiaoyou" / "mcp-tokens.json"
        else:
            self._path = Path.home() / ".suxiaoyou" / "mcp-tokens.json"
