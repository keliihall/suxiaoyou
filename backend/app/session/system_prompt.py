...
def default_now() -> datetime:
    """Return the app-local wall-clock time."""
    return datetime.now(APP_TIME_ZONE_INFO)


def default_tz_name() -> str:
    """Return the app's default timezone name."""
...
def load_project_instructions(directory: str | None) -> str | None:
    ...
    candidates = [
        ...
        os.path.join(directory, ".suxiaoyou", "instructions.md"),
        os.path.join(directory, ".suxiaoyou", "instructions"),
    ]
...
- Current year: {now.year}"""
    ...
    if workspace:
        output_dir = str(Path(workspace) / "suxiaoyou_written")
        section += f"""
