...
_SESSION_TOKEN = "suxiaoyou_st_test_session_abcdef0123456789"
_REMOTE_TOKEN = "suxiaoyou_rt_test_remote_abcdef0123456789"
...
class TestMandatoryAuth:
    ...
    async def test_api_rejects_wrong_token(self):
        ...
        async with await _client(app) as c:
            r = await c.get(
                "/api/ping",
                headers={"authorization": "Bearer suxiaoyou_st_wrong_token_value"},
            )
        assert r.status_code == 401
