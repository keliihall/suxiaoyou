...
def test_ensure_session_token_accepts_dev_override(tmp_path):
    ...
    token = "suxiaoyou_st_test_dev_override"
...
def test_ensure_session_token_rejects_non_session_override(tmp_path):
    with pytest.raises(ValueError, match="suxiaoyou_st_"):
        ensure_session_token(tmp_path / "session_token.json", token="suxiaoyou_rt_wrong")
