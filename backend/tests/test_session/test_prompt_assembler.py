...
class TestDefaultTimezone:
    def test_defaults_to_shanghai_timezone(self) -> None:
        now = default_now()

        assert default_tz_name() == "Asia/Shanghai"
...
class TestWorkspaceVsNoWorkspace:
    def test_workspace_set_emits_restriction(self) -> None:
        ...
        assert "suxiaoyou_written" in parts.dynamic
...
class TestLoadProjectInstructionsHelper:
    ...
    def test_first_match_wins(self, tmp_path) -> None:
        # AGENTS.md takes precedence over .suxiaoyou/instructions.md.
        (tmp_path / "AGENTS.md").write_text("from-agents-md")
        (tmp_path / ".suxiaoyou").mkdir()
        (tmp_path / ".suxiaoyou" / "instructions.md").write_text("from-instructions-md")
        result = load_project_instructions(str(tmp_path))
