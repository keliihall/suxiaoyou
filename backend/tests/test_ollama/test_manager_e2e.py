...
Run explicitly with:
SUXIAOYOU_RUN_OLLAMA_E2E=1 pytest tests/test_ollama/test_manager_e2e.py -q
"""
...
pytestmark = pytest.mark.skipif(
    os.environ.get("SUXIAOYOU_RUN_OLLAMA_E2E") != "1",
    reason="set SUXIAOYOU_RUN_OLLAMA_E2E=1 to download Ollama and a small Qwen model",
)
    ...
    async with httpx.AsyncClient(timeout=180.0) as client:
        resp = await client.post(
            ...
            json={
                "model": model,
                "stream": False,
                "messages": [{"role": "user", "content": "Reply with exactly: 苏小有 Ollama OK"}],
